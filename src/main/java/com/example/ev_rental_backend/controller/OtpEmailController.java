...
import com.example.ev_rental_backend.service.otp.OtpEmailService;
...
public class OtpEmailController {
    ...
    public ResponseEntity<ApiResponse<?>> sendOtp(@RequestBody @Valid ForgotPasswordRequestDTO dto) {
        ...
        try {
            otpEmailService.sendOtpByEmail(email);
    ...
    public ResponseEntity<ApiResponse<?>> verifyOtp(
        ...
        try {
            boolean verified = otpEmailService.verifyOtpByEmail(email, otpCode);
            return ResponseEntity.ok(ApiResponse.<String>builder()
                    .status("success")
                    .code(200)
